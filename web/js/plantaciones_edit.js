(function() {
  $(document).ready(function() {
    ...
    $("#titulares-modal-select").click(function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        titular = _ref[_i];
        $("#plantaciones_editar_plantacion_titular_id").val(titular.value);
        $("#plantaciones_editar_titular").val($($(titular).parent().siblings()[0]).text());
      }
    ...
    $("#remove-titular").click(function() {
      $("#plantaciones_editar_titular").val('');
      return $("#plantaciones_editar_titular").val('');
    });
...
+ '</td>')).append($('<td>' + ((_ref1 = titular.cuit) != null ? _ref1 : '') + '</td>'))));
    ...
    $("#remove-especie").click(function() {
      ...
      _ref = $("#plantaciones_editar_especie option:selected");
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        especie = _ref[_i];
    ...
    $("#especies-modal-add").click(function() {
      ...
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
        especie = _ref[_i];
        $("#plantaciones_editar_especie").append($("<option value='" + especie.value + "'>" + $($(especie
    ...
    $("#plantaciones_editar_provincia").change(function() {
      $("#plantaciones_editar_departamento").prop('disabled', true);
      $("#plantaciones_editar_departamento").empty();
      $("#plantaciones_editar_departamento").append($("<option />"));
      if ($("#plantaciones_editar_provincia").val()) {
        return $.ajax({
          url: "/provincias/" + $("#plantaciones_editar_provincia").val() + "/departamentos.json"
        }).done(function(data) {
          var departamento, _i, _len, _ref;
          _ref = $(data);
          for (_i = 0, _len = _ref.length; _i < _len; _i++) {
            departamento = _ref[_i];
            $("#plantaciones_editar_departamento").append($("<option />").val(departamento.id).text(departamento
.nombre));
          }
          return $("#plantaciones_editar_departamento").prop('disabled', false);
        });
      } else {
        return $("#plantaciones_editar_departamento").prop('disabled', false);
      }
 ...
 del formulario */
    $("form").submit(function(e) {
      $("#plantaciones_editar_especie option").prop('selected', true);
      return $("#plantaciones_editar_historico option").prop('selected', true);
    });
    ...
    $("#plantaciones-modal-add").click(function() {
      $.each($('#plantaciones_editar_nuevas_ids').val().split('\n'), function(key, value) {
        if (value !== '') {
          return $('#plantaciones_editar_historico').append("<option value='" + value + "'>" + value
 + "</option>");
        }
      });
    ...
    $("#remove-nueva-plantacion").click(function() {
      ...
      _ref = $("#plantaciones_editar_historico option:selected");
      _results = [];
      for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    ...
    return $('#nuevas-plantaciones-modal').on('shown.bs.modal', function() {
      return $('#plantaciones_editar_nuevas_ids').focus();
    });
  });
